        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 shadow-2xl sm:rounded-3xl relative overflow-hidden flex flex-col min-h-[500px]">
                ...
                <div class="flex-grow flex flex-col items-center justify-center p-8 space-y-12">
                    ...
                    <template x-if="players.length > 0 && gameState !== 'deadly'">
                        <div class="w-full max-w-sm flex flex-col items-center space-y-12 text-center">
                            <!-- Turning Player -->
                            <div class="space-y-1">
                            ...
                            <div class="relative w-64 h-64 flex items-center justify-center">
                                ...
                                <div class="relative w-48 h-48 transition-transform duration-700 cubic-bezier(0.34, 1.56, 0.64, 1)" :style="'transform: rotate(' + (chambersCompleted * 60) + 'deg)'">
                                    <template x-for="i in 6">
                                        <div 
                                            ...
                                            :class="i <= chambersCompleted ? 'bg-gray-800 border-gray-900' : 'bg-white dark:bg-gray-600 border dark:border-gray-500'"
                                            :style="'transform: rotate(' + ((i-1) * 60) + 'deg) translateY(-60px)'"
                                        >
                                            <template x-if="i <= chambersCompleted">
                            ...
                            <div class="w-full flex flex-col space-y-3">
                                ...
                                <p class="text-[10px] font-bold text-gray-400 uppercase tracking-tighter italic">Risk the 1 in <span x-text="6-chambersCompleted"></span> chance or pass it on</p>
                            </div>
                        </div>
                    </template>
                </div>
                ...
                <template x-if="gameState === 'deadly' || gameState === 'overall_winner'">
                    <div class="absolute inset-0 z-50 bg-black flex flex-col items-center justify-center p-8 text-white overflow-hidden text-center sm:rounded-3xl">
                        ...
                        <div class="mb-12">
                            ...
                            <template x-if="gameState === 'deadly'">
                                <div>
                                    <h2 class="text-xs font-black uppercase tracking-[1em] text-rose-500 animate-pulse mb-4">CRITICAL HIT</h2>
                                    ...
                                    <h2 class="text-3xl font-black text-rose-600 italic tracking-widest mt-2" x-text="lmsActive ? 'IS ELIMINATED!' : 'IS DOWN!'"></h2>
                                </div>
                            </template>
                        </div>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('rouletteGame', () => ({
                ...
                init() {
                    this.loadedSlug = Math.floor(Math.random() * 6); // 0-5
                    console.log('slug at:', this.loadedSlug);
                ...
                pullTrigger() {
                    ...
                    // Small delay to simulate tension/sound
                    setTimeout(() => {
                        if (this.chambersCompleted === this.loadedSlug) {
                            this.killShot();
                        } else {
                            this.safeClick();
                        }
                ...
                safeClick() {
                    ...
                    // Player stays - doesn't move to next until handover
                },

                handover() {
                    if (this.busy) return;
                    this.nextPlayer();
                    // Reset tension for next player? (optional)
                    // If all 5 safe chambers are gone, last one is deadly, handover might be disabled or forced?
                    // But we keep it simple as requested.
                },
                ...
                resetGame() {
                    ...
                    this.loadedSlug = Math.floor(Math.random() * 6);
                    this.logs = [];
                    // Advance starter for next turn
                    this.nextPlayer();
                },
