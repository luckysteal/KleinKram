        ...
        <div class="flex-grow flex flex-col w-full h-full">
            <div class="flex-grow flex flex-col bg-white dark:bg-gray-800 transition-colors duration-300 relative overflow-hidden">
                ...
                <div class="p-8 flex-grow flex flex-col items-center justify-center space-y-8">
                    ...
                    <div x-show="players.length > 0" :class="gameState !== 'playing' ? 'invisible pointer-events-none' : ''" class="w-full max-w-md space-y-12 text-center transition-opacity duration-300">
                        <!-- Current Player -->
                        <div class="space-y-2">
                ...
                <div x-show="gameState === 'picked' || gameState === 'overall_winner'" 
                    x-transition:enter="transition ease-out duration-300" 
                    x-transition:enter-start="opacity-0 scale-95" 
                    x-transition:enter-end="opacity-100 scale-100" 
                    class="absolute inset-0 z-50 bg-white/95 dark:bg-gray-800/95 overflow-y-auto p-4 sm:p-8 text-center shadow-2xl backdrop-blur-md transition-colors duration-300">
                    ...
                    <div class="relative mb-8 sm:mb-12">
                        ...
                        <div class="relative w-28 h-28 sm:w-40 sm:h-40 bg-white dark:bg-gray-700 rounded-full flex items-center justify-center border-4 sm:border-8 border-rose-500 shadow-[0_30px_60px_rgba(225,29,72,0.3)]">
                            <i class="fas" :class="gameState === 'overall_winner' ? 'fa-crown text-amber-500 text-4xl sm:text-7xl' : 'fa-times text-4xl sm:text-7xl text-rose-500'"></i>
                        </div>
                    </div>
                    ...
                    <div class="mb-8 sm:mb-12">
                        <template x-if="gameState === 'overall_winner'">
                            <div>
                                <h2 class="text-[10px] sm:text-xs font-black uppercase tracking-[0.5em] sm:tracking-[1em] text-amber-500 mb-4 px-2">{{ __('ULTIMATE CHAMPION') }}</h2>
                                <div class="text-4xl sm:text-7xl font-black text-amber-600 italic tracking-tighter uppercase drop-shadow-[0_5px_15px_rgba(251,191,36,0.3)]" x-text="winnerName"></div>
                                <p class="text-gray-500 dark:text-gray-400 text-base sm:text-lg mt-4 font-bold opacity-80 uppercase tracking-widest italic">{{ __('Last Man Standing!') }}</p>
                            </div>
                        </template>
                        <template x-if="gameState === 'picked'">
                            <div>
                                <h2 class="text-[10px] sm:text-xs font-black uppercase tracking-[0.5em] sm:tracking-[1em] text-rose-500 mb-4 px-2">{{ __('Wrong Guess') }}</h2>
                                <div class="text-4xl sm:text-7xl font-black text-rose-500 italic tracking-tighter uppercase drop-shadow-[0_5px_15px_rgba(225,29,72,0.3)]" x-text="lastPickedPlayer"></div>
                                <p class="text-gray-500 dark:text-gray-400 text-base sm:text-lg mt-4 font-bold opacity-80 uppercase tracking-widest italic" x-text="lmsActive ? '{{ __('You are Eliminated!') }}' : '{{ __('Prepare your wallet!') }}'"></p>
                            </div>
                        </template>
                    </div>
                    ...
                    <div class="w-full max-w-sm space-y-4">
                        <template x-if="gameState === 'overall_winner'">
                            <button @click="resetRound()" class="w-full py-4 sm:py-6 bg-amber-500 text-white font-black text-xl sm:text-2xl rounded-2xl sm:rounded-3xl shadow-[0_20px_40px_rgba(79,70,229,0.3)] transition-all hover:scale-105 uppercase tracking-tighter">
                                {{ __('NEW ROUND') }}
                            </button>
                        </template>
                        <template x-if="gameState === 'picked'">
                            <div>
                                <div class="text-gray-400 text-[10px] sm:text-xs font-bold uppercase tracking-widest mb-4">
                                    {{ __('The number was') }} <span class="text-gray-800 dark:text-white" x-text="nextNumber"></span>
                                </div>
                                <button @click="resetGame()" class="w-full py-4 sm:py-6 bg-indigo-600 hover:bg-indigo-500 text-white font-black text-xl sm:text-2xl rounded-2xl sm:rounded-3xl shadow-[0_20px_40px_rgba(79,70,229,0.3)] transition-all hover:scale-105 uppercase tracking-tighter">
