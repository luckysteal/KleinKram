<x-app-layout>
    <div class="flex-grow flex flex-col w-full relative" x-data="jumpGame()"
        ...
        <div class="flex-grow flex flex-col w-full h-full">
            <div class="flex-grow flex flex-col bg-white dark:bg-gray-800 transition-colors duration-300 relative min-h-[500px]">
                ...
                <x-game-header reset="resetGame()">
                    <h3 class="text-[13px] min-[360px]:text-base sm:text-2xl font-black text-purple-600 dark:text-purple-500 uppercase tracking-tighter italic truncate">{{ __('Schlossgraben Jump') }}</h3>
                    <p class="text-[8px] sm:text-[10px] text-gray-400 dark:text-gray-500 uppercase tracking-[0.2em] font-bold truncate">{{ __('Jump over the moat!') }}</p>
                </x-game-header>
    ...
    <!-- Web Audio API and Game Canvas Script -->
    <script>
            ...
            Alpine.data('jumpGame', () => ({
                ...
                setupLevel() {
                    ...
                    if (layoutType === 0) {
                        ...
                        this.platforms.push({ x: rightStart, y: groundY, width: 700, height: canvasHeight - groundY + 100, type: 'end' });
                        ...
                        this.level.totalWidth = rightStart + 700;
                ...
                onJumpRelease() {
                    ...
                    // If moving upwards (negative vy), cut off upward velocity to make jump shorter
                    if (p.vy < -3.5) {
                        p.vy = -3.5;
                    }
                },
                ...
                onJumpTrigger() {
                    if (this.gameState !== 'playing' || this.winTimerStarted) return;
                ...
                updatePhysics() {
                    ...
                    if (this.gameState === 'playing') {
                        ...
                        if (p.x >= this.level.rightStart + 120) {
                            if (!this.winTimerStarted) {
                                this.winTimerStarted = true;
                                setTimeout(() => {
                                    if (this.gameState === 'playing' && this.winTimerStarted) {
                                        this.triggerSuccess();
                                    }
                                }, 800);
                            }
                        ...
                        // Prevent running off the ending platform
                        if (this.winTimerStarted && p.x >= this.level.rightStart + 600) {
                            p.vx = 0;
                        }
                ...
                draw() {
                    ...
                    // 4. BACKGROUND IMAGE (with caching, cover scale, and parallax scrolling)
                    if (hasBg) {
                        const factor = 0.08;
                        let cacheKey = `${bgIndex}_${canvasHeight}_${this.level.totalWidth}`;
                        let cached = this.cachedBgs[cacheKey];
                        
                        if (!cached) {
                            let maxCamX = Math.max(1, this.level.totalWidth - 800);
                            let minBgWidth = 800 + maxCamX * factor;
                            let scale = Math.max(minBgWidth / bgImg.naturalWidth, canvasHeight / bgImg.naturalHeight);
                        ...
                        let maxCamX = Math.max(1, this.level.totalWidth - 800);
                        let minBgWidth = 800 + maxCamX * factor;
                        let bgX = -this.camX * factor - (cached.width - minBgWidth) / 2;
                        let bgY = canvasHeight - cached.height;
